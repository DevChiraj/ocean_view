...
public class ReservationDAO {
    ...
    public List<Map<String, String>> getAllRooms() {
        ...
        } catch (SQLException e) { LOGGER.log(Level.SEVERE, "Error fetching rooms", e); }
        return rooms;
    }
    ...
    // 3. බිල්පත සඳහා විස්තර ලබා ගැනීම
    public Map<String, Object> getBillingDetails(String resId) {
        ...
        } catch (SQLException e) { LOGGER.log(Level.SEVERE, "Billing error", e); }
        return data;
    }

    // 4. Dashboard Stats (Cards සඳහා දත්ත)
    ...
    // 5. Monthly Revenue (Chart එක සඳහා දත්ත)
    public Map<String, Double> getMonthlyRevenue() {
        ...
        try (Connection conn = DBConnection.getInstance().getConnection();
             Statement st = conn.createStatement();
             ResultSet rs = st.executeQuery(sql)) {
            while (rs.next()) { revenueData.put(rs.getString("month"), rs.getDouble("total")); }
        } catch (SQLException e) { LOGGER.log(Level.SEVERE, "Revenue chart error", e); }
        return revenueData;
    }

    // 6. මෑතකදී කළ රෙසර්වේෂන් ලබා ගැනීම (නිවැරදි කළා)
    public List<Map<String, String>> getRecentReservations(int limit) {
        List<Map<String, String>> list = new ArrayList<>();
        String sql = "SELECT r.res_id, r.guest_name, rm.room_type FROM reservations r " +
                "JOIN rooms rm ON r.room_no = rm.room_id ORDER BY r.res_id DESC LIMIT ?";
        try (Connection conn = DBConnection.getInstance().getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setInt(1, limit);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
                ...
                map.put("res_id", rs.getString("res_id"));
                map.put("guest_name", rs.getString("guest_name"));
                map.put("room_type", rs.getString("room_type"));
                list.add(map);
            }
        } catch (SQLException e) { LOGGER.log(Level.SEVERE, "Recent data error", e); }
        return list;
    }

    // 7. අනෙකුත් මෙතඩ්ස් (List, Search, Checkout)
    ...
    public Map<String, String> searchReservation(String term) {
        ...
        try (Connection conn = DBConnection.getInstance().getConnection();
            ...
            if (rs.next()) {
                data.put("res_id", rs.getString("res_id"));
                data.put("guest_name", rs.getString("guest_name"));
                data.put("room_no", rs.getString("room_no"));
                data.put("room_type", rs.getString("room_type"));
            }
        } catch (SQLException e) { LOGGER.log(Level.SEVERE, "Search error", e); }
        return data;
    }
